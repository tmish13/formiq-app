"""
Feature extraction service for squat form analysis.

Adapts the ML training codebase feature extraction pipeline to work with 
MediaPipe pose landmarks in the production backend environment.
"""
...
class SquatFeatureExtractor:
    """
    Extracts biomechanical features from MediaPipe pose landmark sequences for squat analysis.
    
    Converts MediaPipe pose data to the 23 features expected by the trained XGBoost model.
    """
    ...
    def _extract_joint_angles_per_frame(self, frame: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            # Knee valgus (simplified frontal plane estimation)
            # Note: This is an approximation since we don't have true frontal plane view
            if all(joint in frame for joint in ['left_hip', 'left_knee', 'left_ankle']):
                ...
                if np.linalg.norm(line_vec) > 1e-6:
                    ...
                    # Convert to approximate angle (this is a simplification)
                    angles['left_knee_valgus_deg'] = lateral_deviation * 100  # Scale factor
            ...
            if all(joint in frame for joint in ['right_hip', 'right_knee', 'right_ankle']):
                ...
                if np.linalg.norm(line_vec) > 1e-6:
                    ...
                    angles['right_knee_valgus_deg'] = lateral_deviation * 100
            
            # Foot flat indicators (simplified)
            if all(joint in frame for joint in ['left_heel', 'left_foot_index']):
                heel_y = frame['left_heel'][1]
                toe_y = frame['left_foot_index'][1]
                # If heel and toe are roughly at same height, foot is flat
                angles['left_foot_flat'] = 1.0 if abs(heel_y - toe_y) < 0.02 else 0.0
            
            if all(joint in frame for joint in ['right_heel', 'right_foot_index']):
                heel_y = frame['right_heel'][1]
                toe_y = frame['right_foot_index'][1]
                angles['right_foot_flat'] = 1.0 if abs(heel_y - toe_y) < 0.02 else 0.0
    ...
    def extract_features(self, pose_sequence: List[Optional[List[Optional[Dict[str, Any]]]]]) -> Dict[str, float]:
        """
        Extract the 23 biomechanical features required by the trained model.
        
        Args:
            pose_sequence: MediaPipe pose sequence from video processing
            
        Returns:
            Dictionary containing the 23 features expected by the XGBoost model
        """
        try:
            ...
            features = self._calculate_model_features(
    ...
    def _calculate_model_features(self, knee_angles, hip_positions, torso_angles, hip_angles,
                                knee_valgus_left, knee_valgus_right, ankle_angles,
                                foot_flat_left, foot_flat_right, phases) -> Dict[str, float]:
        """Calculate the specific 23 features expected by the trained model"""
        ...
        # 2. hip_rom_sufficient
        if valid_hip_angles:
            hip_rom = max(valid_hip_angles) - min(valid_hip_angles)
            features['hip_rom_sufficient'] = 1.0 if hip_rom > 30 else 0.0
        else:
            features['hip_rom_sufficient'] = 0.0
        
        # 3. depth_flag
        if valid_knee_angles:
            features['depth_flag'] = 1.0 if min(valid_knee_angles) < 90 else 0.0
        else:
        ...
        # 5. posture_score (0-100 based on torso control)
        posture_score = 100.0
        ...
        # 7. torso_control_flag
        if valid_torso_angles and len(valid_torso_angles) > 2:
            torso_std = np.std(valid_torso_angles)
            features['torso_control_flag'] = 1.0 if torso_std < 10 else 0.0
        else:
            features['torso_control_flag'] = 1.0
        
        # 8. excessive_forward_lean
        features['excessive_forward_lean'] = 1.0 if features['max_torso_lean_angle'] > 30 else 0.0
        ...
        if valid_knee_valgus_left and valid_knee_valgus_right:
            ...
            if max(avg_left, avg_right) > 0:
                ...
                features['asymmetry_flag'] = 1.0 if asymmetry_ratio > 0.05 else 0.0
            else:
        ...
        features['knee_valgus_flag'] = 1.0 if max(max_left_valgus, max_right_valgus) > 5.0 else 0.0
        
        # 12. stability_score (0-100)
        ...
        features['controlled_descent_flag'] = 1.0 if features['descent_duration'] >= 2.0 else 0.0
        
        # 16. overall_score (average of depth, posture, stability)
        depth_score = 50.0 if features['depth_flag'] == 1.0 else 0.0
        features['overall_score'] = (depth_score + features['posture_score'] + features['stability_score']) / 3
        ...
        features['right_knee_valgus_flag'] = 1.0 if max_right_valgus > 5.0 else 0.0
        ...
        if phases['ascent'] and len(knee_angles) > 0:
            ...
            if len(ascent_knee_angles) > 3:
                ...
                features['smooth_ascent_flag'] = 1.0 if np.std(diffs) < 5 else 0.0
            else:
                features['smooth_ascent_flag'] = 1.0
        else:
            features['smooth_ascent_flag'] = 1.0
