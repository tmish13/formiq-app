...
// Define interfaces for our inventory
...
try {
  ...
  const inventory: Inventory = {
    ...
    importGraph: new Map<string, Set<string>>(),
...
function processImports(sourceFile: SourceFile, fileInfo: FileInfo, inventory: Inventory) {
  try {
    ...
    importDeclarations.forEach((importDecl) => {
      ...
      if (!inventory.importGraph.has(fileInfo.path)) {
        inventory.importGraph.set(fileInfo.path, new Set<string>());
      }
...
function processExports(sourceFile: SourceFile, fileInfo: FileInfo, inventory: Inventory) {
  try {
...
function processComponents(sourceFile: SourceFile, fileInfo: FileInfo) {
  try {
    const filePath = sourceFile.getFilePath();
...
function processHooks(sourceFile: SourceFile, fileInfo: FileInfo) {
  try {
    const filePath = sourceFile.getFilePath();
...
function processSlices(sourceFile: SourceFile, fileInfo: FileInfo) {
  try {
    const filePath = sourceFile.getFilePath();
...
function processServices(sourceFile: SourceFile, fileInfo: FileInfo) {
  try {
    const filePath = sourceFile.getFilePath();
...
function processContexts(sourceFile: SourceFile, fileInfo: FileInfo) {
  try {
    const filePath = sourceFile.getFilePath();
...
function updateInventory(fileInfo: FileInfo, inventory: Inventory) {
...
function detectUnusedFiles(project: Project, inventory: Inventory): string[] {
  try {
