...
| **G-02** | **No retry actually happens.** `max_retries=3` is declared at `analysis_tasks.py:437` but `self.retry()` is never called; every exception is swallowed → FAILED. Effective retries: **0**. | "Fault-tolerant inference pipelines" is on your CV. Reading 40 lines of the task disproves it. | **S** | Re-raise retryable exceptions (S3/DB/transient) so Celery retries with backoff; keep terminal errors non-retryable. Add a test asserting retry count. |
| **G-03** | **`acks_late` = 0 hits. Worker death loses the message and strands the row at `PROCESSING` forever** — no reaper, no DLQ, empty beat schedule. | Standard senior-level queue question: "what happens if the worker dies mid-task?" The honest answer today is "the work is lost and the user waits forever." | **M** | Set `task_acks_late=True` + `task_reject_on_worker_lost=True`; add a beat job to re-queue rows stuck in PROCESSING past a TTL. Add a kill-mid-task test asserting retry + correct final result. |
...
| **G-10** | **No idempotency on upload.** No content hash, no `Idempotency-Key`; timestamped keys. The status guard at `analysis_tasks.py:486` is a read-then-write race. | Classic system-design follow-up to "what if the client retries?" | **M** | Hash file content → unique constraint or dedupe lookup; add `SELECT … FOR UPDATE` (or a conditional UPDATE) on the PENDING→PROCESSING transition. Test: submit the same file twice, assert one result. |
...
| **G-13** | **No end-to-end pipeline test.** `test_full_flow.py` stops at `status == "pending"`; the Celery task, inference, and final result are never asserted. | "How do you know the pipeline works?" — currently unit tests plus manual scripts. | **M** | Add an e2e test that runs the task eagerly (or against a live worker) with a real short video and asserts COMPLETED + a `results.posture_v1.decision`. |
...
| **G-19** | **Dead parallel pipeline:** `video_tasks.py` / `ai_tasks.py` define four `async def` Celery tasks under plain `@app.task` — the prefork pool never awaits them. Reachable from `videos.py:265` and `analysis.py:51`. | If they browse `app/tasks/` first, they'll read the broken pipeline and think it's the real one. | **S** | Move them to `backend/archive/` (which already exists) or delete, and remove the endpoints that dispatch them. |
...
| **G-24** | **The manifest's `feature_order` does not describe the serving code.** It documents features 0-65 as `joint{0..32}_x_mean` / `_x_std` (33 joints, x only); `features_151d.get_feature_names()` actually emits `joint{0..10}_{x,y,z}_mean` / `_std` (11 joints × 3 coords). Both are 33 values, so the 151 total lines up and nothing crashes. Blocks 3-5 do match. | The manifest is the **only** written record of what training consumed, and for 66 of 151 features it demonstrably does not describe the code. If training used the documented layout, those 66 features are misaligned — which would dwarf G-21. Cannot be resolved from this repo. | **M** | Recover the training notebook's feature builder and diff it against `features_151d.py`. Until then, `tests/unit/test_train_serve_parity.py` pins the real layout so drift is caught. |
| **G-25** | **Residual eval gap after G-21.** With complexity 2 restored, `prob_fault` on `T6ad8Et3C5Q_good_rep_1.mp4` is **0.3347** vs the eval CSV's **0.0001** for the same file. Direction is right (margin to threshold went 0.051 → 0.190, confidence 6.4×) but the magnitude is not explained. | You can now say *what caused part of* the 0.72 → 0.18 gap and prove it. You cannot yet say the gap is closed — and an interviewer will ask. | **M** | Bisect: run the eval script directly on the file vs through the upload pipeline, compare `sequence_length` (90 vs 89), MediaPipe version, and aarch64-vs-x86 numerics. One focused session before Day 6. |

**Suggested Phase 2 order (highest risk first, cheap wins bundled):**
`G-01` · `G-02` + `G-03` (one branch, failure-handling) · `G-04` + `G-05` (one branch, feedback eval + deps) · `G-06` + `G-07` + `G-08` (cheap credibility) · `G-09` + `G-12` (one branch, instrumentation) · `G-10` · `G-14`.

---

*End of Phase 1. No files were modified. Awaiting "go" and your selection of gaps before any Phase 2 work begins.*
