...
| **G-28** | **The shortfall is precision-shaped: same true positives, ~31 extra false positives.** Reconstructing the manifest's confusion matrix from its own P/R on 86 positives gives TP~61 FP~21; measured is TP=61 FP=52. Recall reproduces almost exactly (0.7093 vs 0.7051), precision collapses (0.5398 vs 0.7432). The one-video diff points the same way — eval CSV 0.0001 vs serving 0.3347, and a `sequence_length` sweep 60–300 spans [0.147, 0.617], so 0.0001 is unreachable. **Serving skews high on `prob_fault`.** | This is now the single open question behind every accuracy claim. It is also a much sharper interview answer than "the eval set was different": the model finds the same faults, it additionally flags 31 good squats. | **M** | Candidates, untested: a different checkpoint; a different feature extractor (note `fixed_cells_v3_v8_v10_...ipynb` builds 87D from **body joints**, not face coords); a calibration step before thresholding; keypoints from a different pose pass. Cheapest discriminator: run the original eval path on 5 test videos and compare per-video `prob_fault`. |
