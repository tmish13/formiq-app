...
- **Registration is conditional:** `if cache_service and hasattr(cache_service,'redis') and cache_service.is_available()` (`main_setup.py:58`). **2026-09-24: this guard has never been true — `CacheService` has neither a `redis` attribute nor `is_available()`, and the check runs before Redis connects — so the rate-limit middleware has never been installed (`bench/results/2026-09-24-cache-is-the-db.md` §Flagged).** If Redis is down at boot, the `else` branch logs *"Redis unavailable … rate limiting disabled!"* (`main_setup.py:84`) and **the API runs with no rate limiting at all**.
...
- **`/metrics` endpoint:** the only live one is `backend/app/api/v1/endpoints/health.py:317-340`, i.e. **`/api/v1/health/metrics`**, returning rate-limit metrics via `HealthService`. **2026-09-24: it returned 500 on every call (`HealthService()` without arguments, a method that did not exist); now `generate_latest()` at `/health/metrics` (root; the `api/v1` include doubles the prefix), `bench/results/2026-09-24-health-routes.md`.** Prometheus config scrapes it at `infrastructure/monitoring/prometheus/prometheus.yml`. There is **no root `/metrics`** exposing the default registry.
...
| # | Gap | Why an interviewer would care | Effort | Proposed fix |
|---|---|---|---|---|
| **G-55** ✅ **FIXED 2026-09-24 (removed with the approved plan)** | **Two TLS private keys are committed** (`infrastructure/docker/nginx/ssl/api.formiq-app.com.key`, `formiq-app.com.key`, added 2025-04-11). Self-signed (issuer = subject, CN=formiq-app.com), expired 2026-04-10, referenced only by the deleted nginx compose stack. | Private keys in git are a reflex finding for any reviewer, even when, as here, they protect nothing and no one trusts them. Removing them from the tree does not remove them from history. | **S** | `git rm` the `ssl/` directory and say in `infrastructure/README.md` that certificates are issued at deploy time; if the domain is ever served for real, generate new keys — never reuse these. Not done without asking. |
| **G-56** ⚠️ **FILED 2026-09-24 — not fixed (outside the approved scope)** | **The frontend test suite has never been green.** First recorded run: 17 of 32 suites and 94 of 587 tests fail on the committed tree (80 × `is not a function` from mocked services whose interface moved, missing `data-testid`s, a `jsonwebtoken` dev dependency the suite assumes); `tsc --noEmit` fails on one `types` entry (`@testing-library/jest-dom` v6). Two of the three frontend workflows never reached the tests, and the third ran jest from the wrong directory, so nothing ever reported it. | The frontend has no working regression net; every UI change today is verified by hand. | **M** | Repair or retire suite by suite (the 15 green suites stay as the gate meanwhile); fix the `types` entry. `.github/workflows/frontend.yml` now runs Typecheck and Jest as separate jobs so the red one is visible. Evidence `bench/results/2026-09-24-frontend-tests.md`. |
| **G-57** ⚠️ **FILED 2026-09-24 — measured, not fixed** | **The localisation claim was never checked against the dataset's own timing labels.** Fitness-AQA labels every knees-forward error as a `[start_s, end_s]` interval (1,109 of 1,623 videos); the repo only ever used `len(intervals) > 0`. Checked on train+validation (no test read): the shipped rule's `peak_time_sec` falls inside a labelled interval in 238/330 = 72.1 % [67.0, 76.7] of OBSERVED videos, against a 59.5 % chance level (the intervals cover 41 % of a clip); it fires on 51.5 % of videos with an interval and 32.0 % without. The "80.2 % within-video agreement" behind §A3 is repeatability across pose passes, not accuracy. | The one thing the app still asserts per video ("knees went forward at 1.4 s") is ~12 points above chance against ground truth. | **S** to measure properly, **M** to fix | Evaluate per frame (interval IoU / hit rate) on train/validation with `bench/interval_labels.py` as the seed; re-word the §A3 claim to what is measured; consider the interval-derived label as an extra pre-registered Gate-2 arm (validation: presence AUROC 0.745, longest-interval ≥ 2 s 0.782, CIs overlap). Evidence `bench/results/2026-09-24-interval-labels.md`. |
...
| **G-52** ✅ **FIXED 2026-09-24** | **Nine responses in ten carried the correlation id `0000000000000000`.** `core/middleware/trace_context.py` handed out the OTel-derived correlation id whenever tracing was enabled, and for an *unsampled* request (the default sampler keeps 10 %) that id is all zeros; the per-request uuid fallback was never reached. Found because the concurrent-requests test had five requests sharing one "unique" id since the middleware was written (G-35). | A correlation id that is the same for 90 % of requests is not a correlation id; every incident investigation that used it was matching the wrong requests. | **S** | Done — all-zero is treated as absent and the uuid is generated; an incoming `X-Correlation-ID` still wins; three tests. |
| **G-53** ✅ **FIXED 2026-09-24** | **The result cache was dead code and the real cache was invisible.** Two Redis cache methods and two write sites in `form_check_service.py` were never written and never read (no video hash was ever supplied; the only hasher hashed the path; `expire=` was not even the client's keyword, so four live callers raised into a broad `except`). What skips inference is the DB idempotency lookup on (user, sha256, model version, spec hash), which returned the old row silently and ignored the pose pass, so a verdict from a superseded MediaPipe pass would be served after the pass changed (G-39: 15.3 % of verdicts flip between passes). | "How many uploads hit the cache" had no answer, and a cache hit could return a measurement the serving pass would not reproduce. | **S** | Done on `audit/cache-is-the-db` — a hit on the same pose pass records an `analysis_runs` row with `pose_source='cache'` copying the source verdict; a hit from another pass re-runs; dead code deleted, `expires_in=` fixed, A2 skips cache rows. Evidence `bench/results/2026-09-24-cache-is-the-db.md`: 20 files twice → 20 real runs, 20 cache rows, 0 cross-pass, 0 second inference; cache row p50 2.28 ms. |
| **G-54** ✅ **FIXED 2026-09-24** | **Every gunicorn worker built the ML stack on its first request.** `app/api/deps.py` constructed `AIService()` (MediaPipe Pose complexity 2, torch, the model loaders) as a per-process singleton the first time a request resolved the form-check service, although the API never runs inference. Measured: one 0.5 MB upload took the serving worker from 315 to 722 MiB, permanently, and 2.03 s; four workers doing it at once under a burst was the OOM kill that backpressure (G-36) and streaming could not remove. | The API's memory was 4× what it needed, every burst killed a worker, and the killed worker's in-flight requests died with empty replies. | **S** | Done on `audit/g54-api-loads-ml` — a `LazyAIService` proxy is injected; the real service is built once per process on first method call. Evidence `bench/results/2026-09-24-g54-api-loads-ml.md`: the same 20-upload burst at 8 slots went from 14 accepted / 6 dropped / 1 kill / 4,656 MiB to 20 / 0 / 0 / 972 MiB; single-upload cost +407 → +4..11 MiB, 2.03 → 0.27 s. |
| **G-38** ✅ **FIXED 2026-09-24** | **`alembic check` was permanently red: 45 operations between the models and a database at head.** 23 were the models' fault (`index=True` on eleven primary keys asking for redundant `ix_<table>_id` indexes; ten indexes and one unique constraint the DB has carried since 0002–0009 that no model declared — autogenerate proposed to *drop the 0009 idempotency key*); 22 were real: six indexes the models always declared and no migration created (three of them UNIQUE — `users.email`, `users.username`, `user_sessions.session_id` — so the app relied on uniqueness the DB never enforced) and sixteen `videos` column comments. | Nobody could tell a real drift from noise, and the schema the code assumed was not the schema that existed. | **M** | Done on `audit/g38-alembic` — models corrected (10 files), one hand-reviewed migration `0013_reconcile_schema` (applied and round-tripped; 0 duplicate emails/usernames/session ids beforehand), `alembic upgrade head && alembic check` in CI. Evidence `bench/results/2026-09-24-g38-alembic.md`: `No new upgrade operations detected.` |
| **G-51** ✅ **MITIGATED 2026-09-24 (`bench/rebuild_images.sh` refuses below 15 GB free and prunes first)** | **Rebuilding three images filled Docker Desktop's 64 GB disk image; the daemon's metadata store then failed every write and the whole stack went dark.** After `docker compose build app worker beat` (2026-09-24 11:58 PDT) `docker system df` reported 0 images (the store was unreadable), `docker exec` and container inspect returned `input/output error` on overlay2 layers, the API answered nothing, and `builder prune` could not write its own metadata. `Docker.raw` sat at exactly 64.0 GB — the cap; the host had 12 GiB free. The old image layers (three services built from one Dockerfile, ~5 GB each) were not pruned before the rebuild and there was no disk headroom check. | The same shape as G-46 and the extractor stall: a resource with no ceiling check fails silently at the ceiling, and the first symptom is somewhere else (an image "not found", a health check "unhealthy"). | **S** | Raise the Docker Desktop disk image size or prune before builds; add `docker system df` + a free-space assertion to the rebuild step (as the extractor already has for its output dir); and never rebuild three copies of one image without pruning the previous three. **Recovered 2026-09-24 12:08 PDT** (user-approved): Docker Desktop restarted, dangling images pruned (3.1 GB), the VM filesystem back to 20 GB used of 58 GB; all five services up, API 200, 3,802 form-check rows intact. |
...
| **G-43** ✅ **FIXED 2026-09-24** | **G-32's fix never reached the running containers.** `pytest-timeout` was added to `requirements.txt` on 2026-09-20 and G-32 was marked FIXED, but `deployment-app` and `deployment-worker` were never rebuilt, so `pytest` inside them still aborts before collection with `ERROR: Missing required plugins: pytest-timeout`. Discovered by accident: the `beat` service added in Stage A triggered a fresh build, and only THAT image (`deployment-beat:latest`) can run the suite. | "Your tests pass — in which image?" A dependency fix that lives in a requirements file and not in the deployed artifact is not a fix, and marking it closed hid that for three days. The same gap applies to any library change: nothing in this setup forces a rebuild or tells you the image is stale. | **S** | Rebuild `app` and `worker`, and add a startup or CI check that the image's installed set matches `requirements.txt`. Until then, run the suite against a freshly built image and say which one. Current reference run: `deployment-beat:latest`, **1274 passed / 2 failed** (the two pre-existing G-35 failures), against the Phase 1 baseline of 1170/2. **2026-09-24:** `app/core/image_check.py` compares every pinned line of `requirements.txt` with what is importable at API and worker start and as a CI step (`audit/image-check`); every worker process now logs at start *image check: 64 pins checked, 1 missing: pytest-timeout* — the truth, stated by the process. The rebuild itself is still pending.  **Rebuilt 2026-09-24 11:58 PDT** (`docker compose build app worker beat`, containers recreated): the image check reports *64 pins checked, 0 mismatched, 0 missing* in all three services at start,. After the G-51 recovery the unit + API suite ran inside `deployment-worker:latest`: **1,382 passed / 2 failed** (the pre-existing G-35 pair) — the image that could not even collect tests for a week now runs them; and the live e2e test passes on the rebuilt stack (2 passed, 18 s).|
