...
| **G-29** | **v1's headline F1 sits at its own trivial floor, and the binary framing hides a production defect.** The curated task is 56% positive, so always-predict-fault scores F1 **0.7197** vs the manifest's 0.7237 (+0.004). AUROC 0.76 shows real ranking signal that F1 at this prevalence cannot express. Separately, v1 flags **46.5% of depth-fault squats as posture faults** — a class it never saw as a negative but which real users produce — so realistic precision is **0.54**, not 0.76. And all 124 Penn Action clips in the corpus are good_form: the external set's faults are a (domain, class) pair with zero training examples, which is why recall there is 0.111. | "What's your model's accuracy?" now has a precise, defensible answer: AUROC 0.76 on the task as trained, 0.72 on a realistic population, F1 indistinguishable from a constant classifier — and you know exactly why. | **L** | v2: train with depth faults in the negative set, include Penn-domain faults, gate on AUROC + F1-CI-above-floor. |
