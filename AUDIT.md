...
|---|---|---|---|---|
| **G-21** | **MediaPipe silently runs at complexity 1, not the configured 2, in every container including production** — the heavy model is downloaded at runtime into root-owned `site-packages` while the process runs as `appuser`. Measured effect on one video: `prob_fault` 0.0001 → 0.4739 against a 0.525 threshold. | This is the strongest single explanation for the 0.72 → 0.18 train/serve gap, and it means the deployed model is not the one that was evaluated. It also invalidates any per-frame timing measured before it is fixed. | **S** | Pre-download `pose_landmark_heavy.tflite` at build time as root in `backend/Dockerfile`, or ship it as a build artifact. Then re-run the 24-video eval and compare. |
| **G-22** | **PostureV1's `StandardScaler` was fitted under scikit-learn 1.6.1 but is unpickled under the pinned 1.4.0**, emitting `InconsistentVersionWarning`. It normalises all 151 features feeding the classifier. | "Are your inference features identical to your training features?" — currently the honest answer is "not provably." | **S** | Align the `scikit-learn` pin to the version the artifact was fitted with, or re-fit and re-serialise the scaler under the pinned version. Assert feature parity in a test. |
