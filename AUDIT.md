...
| **G-25** ⚠️ **REFRAMED 2026-09-19 — moves from pipeline to eval-set investigation** | After G-21, `prob_fault` on one fixture was 0.3347 vs the eval CSV's 0.0001. The face-block hypothesis was tested and **rejected**: ablating all 87 face features costs only **0.052 F1** (`bench/results/2026-09-19-face-block-ablation.md`). More decisively, baseline in-domain F1 is **0.769** against the manifest's recorded test F1 of **0.724** — **serving reproduces training-time performance**. | The 24-video e2e F1 of 0.182 is therefore not a pipeline, parity or feature-design problem. It is a property of that eval set: domain shift and/or labels — the eval report itself flags rows as *"user: actually good form — Penn Action mislabel"*. | **M** | Re-examine the 24-video labels and characterise their distance from the training distribution. Stop chasing pipeline parity. Caveat: the 50 in-domain fixtures may overlap the training split, which would make 0.769 optimistic. |
